...
-- INSERT ALL NCERT CHAPTERS (Dynamically linking subject_id and class_id)
DO $$
DECLARE
...
-- ==========================================================
-- ADD TABLE CONSTRAINTS THAT PRISMA CAN'T DEFINE NATIVELY
-- ==========================================================

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'chk_topic_source'
    ) THEN
        ALTER TABLE tracker.user_tracker 
        ADD CONSTRAINT chk_topic_source 
        CHECK (
...
BEGIN
    ...
    SELECT 
        ...
        ch.class_id,
        ch.id as chapter_id,
        'pending'::tracker."TrackerStatus" as status,
        false as ncert_read
